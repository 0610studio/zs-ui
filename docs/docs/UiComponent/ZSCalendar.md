...
|------|-------|-----------|
| 월간 | 한 달 | 그 달 |
| 주간 | 한 주 | 접힌 그 주는 떠나온 달, 나머지 주는 과반이 속한 달 |
...
접혀 있을 때 남는 주는 **선택일이 보고 있는 달의 날짜일 때만** 그 날이 속한 주입니다. 선택일이 다른 달이면(달만 넘겨본 경우) 그 달의 첫 주가 남습니다. 어느 쪽이든 선택 자체는 그대로 유지되므로, 해당 달로 돌아가면 선택 표시가 다시 보입니다.

달 경계에 걸친 주는 두 달의 그리드에 모두 들어가 있어 주만으로는 어느 달인지 정할 수 없습니다 — `2025-04-27 ~ 05-03` 은 4월이 4일로 과반이지만 5월 그리드의 첫 행이기도 합니다. 그래서 **접히는 순간의 그 한 주에 한해** 떠나온 달을 그대로 이어받습니다. 5월을 보다 접었으면 5월로 남고, 다시 펼쳐도 5월로 돌아옵니다. 주를 한 칸이라도 넘기면 과반 규칙으로 돌아갑니다.
